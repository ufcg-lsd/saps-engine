...
public class TestCrawlerIntegration {
	...
	public void testStepOverImageWhenDownloadFails() throws SQLException, IOException {
		...
		//3. we face a download error for the first. then we step over downloading it
		//4. we are able to download the second one
		//5. in the end, we shall 1 DOWNLOADED and 1 NOT_DOWNLOADED and the pendingDB is empty
		...
		Mockito.doThrow(new IOException()).when(nasaRepository).downloadImage(image1);
		Mockito.doNothing().when(nasaRepository).downloadImage(image2);
		
		Crawler crawler = new Crawler(properties,  imageStore, nasaRepository, federationMember, fmask);
	...
	@Test
	public void testErrorOnFmask() throws SQLException, IOException, InterruptedException {
		...
		//4. image 1 is marked as with error and is put to ERROR state along with an error message
		...
		Mockito.doReturn(fmaskScriptPath).when(properties).getProperty(Crawler.FMASK_SCRIPT_PATH);
		Mockito.doReturn(fmaskToolsPath).when(properties).getProperty(Crawler.FMASK_TOOL_PATH);
		Mockito.doReturn(sebalExportPath).when(properties).getProperty(Crawler.SEBAL_EXPORT_PATH);

		Mockito.doReturn(imageList).when(imageStore).getImagesToDownload(federationMember, maxImagesToDownload);
		Mockito.doNothing().when(nasaRepository).downloadImage(image1);
		Mockito.doReturn(1).when(fmask).runFmask(image1, fmaskScriptPath, fmaskToolsPath, sebalExportPath);
		
		Mockito.doNothing().when(nasaRepository).downloadImage(image2);
		Mockito.doReturn(0).when(fmask).runFmask(image2, fmaskScriptPath, fmaskToolsPath, sebalExportPath);
	...
	@Test
	public void testNoImagesToDownload() throws SQLException, IOException {
		
		// setup
		Properties properties = Mockito.mock(Properties.class);
		ImageDataStore imageStore = Mockito.mock(JDBCImageDataStore.class);
		NASARepository nasaRepository = Mockito.mock(NASARepository.class);
		ConcurrentMap<String, ImageData> pendingImageDownloadMap = Mockito.mock(ConcurrentMap.class);
		Collection<ImageData> data = Mockito.mock(Collection.class);
		FMask fmask = Mockito.mock(FMask.class);
		String federationMember = "fake-fed-member";
		String fmaskScriptPath = "fake-script-path";
		String fmaskToolsPath = "fake-tool-path";
		...
		ImageData image1 = new ImageData("image1", "link1",
				ImageState.DOWNLOADING, federationMember, 0, "NE", "NE",
				date, date, "");
		ImageData image2 = new ImageData("image2", "link2",
				ImageState.NOT_DOWNLOADED, federationMember, 1, "NE", "NE",
				date, date, "");
		...
		String image1FilePath = sebalExportPath + "/" + image1.getName();
		File image1File = new File(image1FilePath);
		
		Crawler crawler = new Crawler(properties,  imageStore, nasaRepository, federationMember, fmask);

		Mockito.doReturn(sebalExportPath).when(properties).getProperty(Crawler.SEBAL_EXPORT_PATH);
		Mockito.doReturn(data).when(pendingImageDownloadMap).values();
		
		Assert.assertEquals(ImageState.DOWNLOADING, image1.getState());
		Assert.assertEquals(ImageState.NOT_DOWNLOADED, image2.getState());
		
		// exercise
		crawler.cleanUnfinishedDownloadedData(properties);

		// expect
		Assert.assertFalse(crawler.isImageOnDisk(image1FilePath, image1File));
		Assert.assertEquals(ImageState.NOT_DOWNLOADED, image1.getState());
		Assert.assertEquals(ImageState.NOT_DOWNLOADED, image2.getState());
	}
