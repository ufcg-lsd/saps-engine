...
public class SapsTask {
	...
	public static TaskImpl createSebalTask(TaskImpl rTaskImpl, Properties properties,
		...
		rTaskImpl.putMetadata(METADATA_MOUNT_POINT, properties.getProperty(SEBAL_MOUNT_POINT));
		...
		String cleanEnvironment = "sudo rm -rf " + properties.getProperty(SEBAL_SANDBOX);
		rTaskImpl.addCommand(new Command(cleanEnvironment, Command.Type.REMOTE));

		String cleanRaster = "sudo rm -rf " + properties.getProperty(SEBAL_RASTER_TMP_DIR);
		...
		String remoteInitScriptPath = rTaskImpl.getMetadata(TaskImpl.METADATA_SANDBOX)
				+ File.separator + localInitScriptFile.getName();

		File localRunScriptFile = createScriptFile(properties, rTaskImpl, RUN_TYPE);
		String remoteRunScriptPath = rTaskImpl.getMetadata(TaskImpl.METADATA_SANDBOX)
				+ File.separator + localRunScriptFile.getName();

		// adding commands
		String scpUploadCommand = createSCPUploadCommand(localInitScriptFile.getAbsolutePath(),
				remoteInitScriptPath);
		LOGGER.debug("ScpUploadCommand=" + scpUploadCommand);
		rTaskImpl.addCommand(new Command(scpUploadCommand, Command.Type.LOCAL));

		scpUploadCommand = createSCPUploadCommand(localRunScriptFile.getAbsolutePath(),
				remoteRunScriptPath);
		LOGGER.debug("ScpUploadCommand=" + scpUploadCommand);
		rTaskImpl.addCommand(new Command(scpUploadCommand, Command.Type.LOCAL));
		...
		rTaskImpl.addCommand(new Command(remoteChmodInitScriptCommand, Command.Type.REMOTE));

		String remoteChmodRunScriptCommand = createChmodScriptCommand(remoteRunScriptPath);
		rTaskImpl.addCommand(new Command(remoteChmodRunScriptCommand, Command.Type.REMOTE));
	...
	private static void settingCommonTaskMetadata(Properties properties, Task task) {
		...
		task.putMetadata(METADATA_MOUNT_POINT, properties.getProperty(SEBAL_MOUNT_POINT));
