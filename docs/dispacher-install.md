...
- The ```$openstack_object_store_service_key``` argument in [Dispatcher configuration file](/config/dispatcher.conf) used to access Object Store must be configured using the command ```swift post -m “Temp-Url-Key:$openstack_object_store_service_key”``` passing as other information as parameters. Use a random 8 chars string and run the following command to configure:
...
Note: ```-auth-version``` is version in operation of the Openstack Identity Service API
